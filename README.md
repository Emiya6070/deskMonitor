...
新增“更多用量”设置：Cursor、Claude、Grok 的个人订阅和 API/团队费用独立开启、独立排序。Claude 订阅通过官方状态栏桥接，API 费用使用各家管理接口；Cursor/Grok 个人订阅目前仅支持本地快照导入。接入步骤、权限及限制见 [用量配置说明](docs/provider-usage.md)。

“外观与性能 → 显示日期进度”可开启周/月日期圆点。“显示位置”可选“左侧 · 竖排”或“卡片下方 · 横排”，旧设置默认保持左侧。横排支持左对齐、居中或逐行拉伸，宽度不足时自动换行，双列模式下铺在两列卡片下方；也可隐藏“本周/月份与已过天数”标题行，只保留日期点。默认红色代表已过日期、绿色代表未过日期，今天用绿色加描边；周一开始，按本机日期自动切换。支持地铁站点式或紧凑网格，可修改圆点大小、间距、标签和颜色。
...
「外观与性能」页可选择八套内置皮肤，保存前提供示例预览：森林绿、石墨、纸白、柔雾紫、冰川，以及新增的午夜蓝、落日和抹茶。行情、消息、用量、窗口和设置页共用主题。设计参考 [Fluent 2 的色彩与材质](https://fluent2.microsoft.design/material)、[Material 3](https://m3.material.io/) 与 [Apple 的玻璃层次设计](https://developer.apple.com/videos/play/wwdc2025/219/)；使用纯色或静态渐变，没有背景模糊和实时折射。
...
- **行情卡片高度与行间距**：可在默认高度基础上从 -12 到 +48 DIP 调整。负值压缩纵向内边距与行间距，正值增加留白和趋势区域；只影响行情卡片，不改变消息与用量卡片。
- **趋势开关与刷新频率**：可隐藏趋势图；界面每 1 / 2 / 5 秒更新，默认 1 秒。调整界面频率不会改变行情连接；曲线正常更新最多每 5 秒一次，切换跨度或调整尺寸时立即重绘。
- 「行情与行为」页管理币种和美股、三档样式、卡片顺序、自启动和吸附；「消息源」页管理 HTTP 消息；「附加显示」页管理 Codex 用量。改变字体缩放或数字大小会重新计算默认窗口尺寸；仅换皮肤保留自定义尺寸。
...
- **双排模式与自由分组**：在「行情与行为」开启“两列显示行情”。默认按排序逐行排列；选中卡片后可指定自动/左列/右列及组名（最多 30 字，留空不分组），保存生效。自定义后两列独立向下排列，同列同组聚合并显示标题，组及组内顺序按卡片排序决定；自动分列的同组卡片跟随该组指定列。关闭双排会合并显示但保留左右配置，重新开启即可恢复。三档样式、消息和用量卡片均支持，日期进度可放左侧或下方。
- **低额度样式**：Codex 及提供剩余百分比的 Cursor/Claude/Grok 指标在剩余 ≤20% 时显示红字，恢复后自动还原；重置时间加粗。只有费用金额的 API 指标不推算剩余额度，等待重置时不显示低额度红字。
- **质感皮肤**：新增黑曜金、暖瓷白、深海钛，使用静态渐层与细边框，无背景模糊或动画开销。在「外观与性能」预览并保存。
...
- **美股行情**：选择“美股”，输入 AAPL、MSFT、NVDA 等代码即可添加。默认使用 Alpaca 官方 WebSocket，设置 API Key ID 和 Secret Key 后可选择免费 IEX 实时、SIP 延迟 15 分钟或付费 SIP 实时；卡片明确显示当前 feed。也可主动切换到 Yahoo Finance 免密实验源（HTTP 每 15 秒刷新），系统不会在 Alpaca 失败时静默降级。所有美股均为 USD 计价，不应作为交易执行依据。Alpaca 凭据保存在当前 Windows 用户的本机设置文件中，请勿共享该文件。
- **HTTP 消息源**：可添加最多 10 个 HTTP/HTTPS GET 消息卡片，直接显示纯文本，或用 `data.items.0.title` 这类点号路径提取 JSON。每个源可独立设置 5 秒至 1 小时的更新频率；失败时保留上一次成功内容并显示错误状态。
- **趋势跨度**：中号卡片右侧、大号曲线上方可单独选择 2 分钟、5 分钟、15 分钟、1 小时、4 小时、1 天、1 周；按每个交易对及现货/永续身份保存，切换不重连行情。图表展示本次运行的本地真实采样，尚未采集的时间留空；重启后重新积累。
- **手动大小与锁定**：开启「允许手动调整大小」后拖动窗口边缘或角落；调整完成再关闭开关，会自动勾选「锁定面板大小」，保存和重新启动仍保留。也可直接勾选锁定当前尺寸。锁定后切换样式、改变字号或增减卡片仍保留宽高，不会因内容换行后重新计算最小尺寸而撑大，也不会被自动尺寸的底部留白缩短；尺寸仍受屏幕范围限制。高度不足时滚动。取消锁定可恢复自动大小，重新开启手动调整也会解除锁定。
...
设置的「附加显示」页开启「显示 Codex 用量卡片」，可与行情同时显示，也可移除全部币种后仅保留用量。卡片同时显示账户额度、本地 Token 总量和估算美元；中、大号增加输入/缓存输入/输出拆分、进度条、重置倒计时和本地重置时间。中、大号点击「刷新」，小号右键「刷新 Codex 用量」可立即查询。

Token 区间可选短周期重置后、长周期重置后、今天、最近 7 个自然日或最近 30 个自然日。设置页会列出每个实际使用模型的 Token、输入/缓存/输出拆分和估算费用；重置点按接口返回的 `resetsAt - windowDurationMins` 计算，接口缺少该窗口时明确回退为今天。
...
币种目录按需流式读取并在本次运行缓存，可手动刷新；不周期 HTTP 轮询。UI 默认每秒刷新一次，曲线每 5 秒最多采样及重建一次，每项只保存最近一周、最多 120961 个点（含边界）；小号宽度不足或关闭趋势图时不创建图表数据数组。用量数据未变化时，仅在显示分钟、过期或重置状态变化时更新；复用冻结的主题画刷。隐藏后停止 UI 刷新和采样，仅保留行情连接，恢复时保留空白区间。无常驻动画、常驻透明分层窗口或浏览器进程。程序依赖的 .NET / WPF / 图形驱动仍有常驻内存开销，实测与限制见 [验证记录](docs/verification.md)。
